...
query_id = "47246b23-d6e4-4055-8942-4b3e0d4bb8cd"
...
# Read Status excel for lookup
status_excel_path = os.path.join(output_dir, "Status excel.xlsx")
print(f"\nReading status excel: {status_excel_path}")
status_df = pd.read_excel(status_excel_path, sheet_name='PT status')
...
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    ...
    uat_status_path = os.path.join(output_dir, "UAT Status Excel.xlsx")
    try:
    ...
    bug_summary_path = os.path.join(output_dir, "Bug_summary.xlsx")
    try:
