...
# ============================================================================
# IMPORTANT: Edit paths in config.py - they will auto-update for all scripts
# ============================================================================
from config import (
    OUTPUT_DIR, 
    POD_MAPPING_PATH,
    PT_STATUS_FILE,
    UAT_DETAILED_REPORT_FILE,
    PO_DETAILS_FILE,
    OPEN_BUG_SUMMARY_FILE,
    STORY_SUMMARY_FILE,
    OVERALL_DEFECT_SUMMARY_FILE,
    DAILY_STATUS_DASHBOARD_FILE
)

# Define paths - scripts are in same folder as this file
script_dir = os.path.dirname(os.path.abspath(__file__))
python_exe = sys.executable

# Scripts to run in order
download_pt_script = os.path.join(script_dir, "download_PT status_file.py")
download_uat_script = os.path.join(script_dir, "download_UAT status_file.py")
script0 = os.path.join(script_dir, "generate_product_owner_details Integrated.py")
script1 = os.path.join(script_dir, "process_uat_status Integrated.py")
script2 = os.path.join(script_dir, "generate_open bug_summary Integrated.py")
script3 = os.path.join(script_dir, "generate_story_summary_detailed Integrated.py")
script4 = os.path.join(script_dir, "generate_overall_defect_summary Integrated.py")
...
print("Step 3: Generating Product Owner Details (prerequisite)...")
print("-" * 80)
result0 = subprocess.run([python_exe, script0], capture_output=False)
if result0.returncode != 0:
    print("Error running generate_product_owner_details Integrated.py")
    sys.exit(1)
print()

# Run process_uat_detailed_report.py
print("Step 4: Processing UAT Detailed Report...")
print("-" * 80)
result1 = subprocess.run([python_exe, script1], capture_output=False)
if result1.returncode != 0:
    print("Error running process_uat_detailed_report.py")
    sys.exit(1)
print()

# Run generate_open bug_summary Integrated.py
print("Step 5: Generating Bug Summary...")
print("-" * 80)
result2 = subprocess.run([python_exe, script2], capture_output=False)
if result2.returncode != 0:
    print("Error running generate_open bug_summary Integrated.py")
    sys.exit(1)
print()

# Run generate_story_summary_detailed Integrated.py
print("Step 6: Generating Story Summary...")
print("-" * 80)
result3 = subprocess.run([python_exe, script3], capture_output=False)
if result3.returncode != 0:
    print("Error running generate_story_summary_detailed Integrated.py")
    sys.exit(1)
print()

# Run generate_overall_defect_summary Integrated.py
print("Step 7: Generating Overall Defect Summary...")
print("-" * 80)
result4 = subprocess.run([python_exe, script4], capture_output=False)
if result4.returncode != 0:
    print("Error running generate_overall_defect_summary Integrated.py")
    sys.exit(1)
print()

# Read generated Excel files
print("Step 8: Reading generated Excel files...")
print("-" * 80)
story_summary_path = STORY_SUMMARY_FILE
bug_summary_path = OPEN_BUG_SUMMARY_FILE
pt_status_path = PT_STATUS_FILE
overall_defect_summary_path = OVERALL_DEFECT_SUMMARY_FILE
...
# Read Milestone Dates from PT Status excel.xlsx - Milestone sheet
milestone_dates_df = pd.read_excel(pt_status_path, sheet_name='Milestone')
...
# Read UAT metrics from UAT Release Detailed Report - UAT Status sheet
# This sheet has a transposed layout: Column A = Metric names, Column B = Values
uat_report_path = UAT_DETAILED_REPORT_FILE
try:
    uat_status_df = pd.read_excel(uat_report_path, sheet_name='UAT Status', header=None)
    # Create a dictionary from the transposed data (Column 0 = names, Column 1 = values)
    uat_metrics = {}
    for idx, row in uat_status_df.iterrows():
        metric_name = str(row.iloc[0]).strip() if pd.notna(row.iloc[0]) else None
        metric_value = row.iloc[1] if len(row) > 1 and pd.notna(row.iloc[1]) else 0
        if metric_name:
            uat_metrics[metric_name] = metric_value
    
    # Extract metrics from dictionary
    uat_total_tests = int(uat_metrics.get('Total UAT Test Cases', 0)) if uat_metrics.get('Total UAT Test Cases') else 0
    uat_passed = int(uat_metrics.get('Passed', 0)) if uat_metrics.get('Passed') else 0
    uat_failed = int(uat_metrics.get('Failed', 0)) if uat_metrics.get('Failed') else 0
    uat_blocked = int(uat_metrics.get('Blocked', 0)) if uat_metrics.get('Blocked') else 0
    uat_not_run = int(uat_metrics.get('Not Run', 0)) if uat_metrics.get('Not Run') else 0
    
    # Execution and Pass percentages are already in percentage format (e.g., 83.00)
    uat_exec_val = uat_metrics.get('UAT Execution %', 0)
    uat_execution_pct = float(uat_exec_val) if uat_exec_val else 0
    
    uat_pass_val = uat_metrics.get('UAT Pass %', 0)
    uat_pass_pct = float(uat_pass_val) if uat_pass_val else 0
except Exception as e:
    print(f"Warning: Could not read UAT Status from new file: {e}")
...
# Group by Parent, Parent Title, SM POC, and M POC to show separate rows for each POC combination
# This ensures that if a Parent has stories from multiple M POCs, they appear as separate rows
execution_summary_df = testable_stories_df.groupby(['Parent', 'Parent Title', 'SM POC', 'M POC']).agg({
...
execution_summary_df.columns = ['Parent', 'Parent Title', 'SM POC', 'M POC', 
...
module_execution_summary_df = testable_stories_df.groupby(['TextVerification', 'SM POC', 'M POC']).agg({
    'ID': ['count', lambda x: ','.join(map(str, x.unique()))],  # Count and list of IDs
    'PT delivered': 'sum',
    'PT NOT delivered': 'sum',
    'UAT Testable Stories': 'sum',
...
# Flatten column names
module_execution_summary_df.columns = ['Module', 'SM POC', 'M POC', 
                                       'Total Stories', 'Story IDs',
                                       'PT delivered', 'PT NOT delivered', 'UAT Testable Stories',
                                       'UAT delivered', 'UAT NOT delivered', 'Passed', 'Failed', 'Blocked', 'Not Run']
...
# Build hierarchy: SM -> M
poc_hierarchy_exec = {}
for _, row in execution_summary_df.iterrows():
    sm_poc = row['SM POC']
    m_poc = row['M POC']
    
    if sm_poc not in poc_hierarchy_exec:
        poc_hierarchy_exec[sm_poc] = []
    if m_poc not in poc_hierarchy_exec[sm_poc]:
        poc_hierarchy_exec[sm_poc].append(m_poc)

# Get unique lists for filters
unique_sm_pocs_exec = sorted(execution_summary_df['SM POC'].unique())
...
for _, row in module_execution_summary_df.iterrows():
    ...
    if sm_poc not in poc_hierarchy_module:
        poc_hierarchy_module[sm_poc] = []
    if m_poc not in poc_hierarchy_module[sm_poc]:
        poc_hierarchy_module[sm_poc].append(m_poc)

unique_sm_pocs_module = sorted(module_execution_summary_df['SM POC'].unique())
...
def get_detailed_poc_breakdown(df):
    ...
    # Create separate breakdowns for SM and M POCs
    sm_breakdown = {}
    m_breakdown = {}
...
for _, row in node_breakdown_df.iterrows():
    ...
    if sm_poc not in poc_hierarchy_defect:
        poc_hierarchy_defect[sm_poc] = []
    if m_poc not in poc_hierarchy_defect[sm_poc]:
        poc_hierarchy_defect[sm_poc].append(m_poc)
...
planned_pt_not_filled = planned_pt_not_filled_filtered.groupby(['SM POC', 'M POC']).size().reset_index(name='Count').sort_values('Count', ascending=False)
...
not_delivered_pt = not_delivered_pt_filtered.groupby(['SM POC', 'M POC']).size().reset_index(name='Count').sort_values('Count', ascending=False)
...
not_delivered_uat = not_delivered_uat_filtered.groupby(['SM POC', 'M POC']).size().reset_index(name='Count').sort_values('Count', ascending=False)
...
if pt_cutoff_date:
    ...
    beyond_pt_cutoff = beyond_pt_cutoff_filtered.groupby(['SM POC', 'M POC']).size().reset_index(name='Count').sort_values('Count', ascending=False)
...
parallel_uat = parallel_uat_filtered.groupby(['SM POC', 'M POC']).size().reset_index(name='Count').sort_values('Count', ascending=False)
...
ccb_approved = ccb_approved_filtered.groupby(['SM POC', 'M POC']).size().reset_index(name='Count').sort_values('Count', ascending=False)
...
print("Step 10: Generating HTML Dashboard...")
print("-" * 80)
...
html_content += """                    </tbody>
            ...
            <h3 style="margin-top: 25px; margin-bottom: 15px; color: #2c3e50;">Defects Breakdown by M POC</h3>
            <div class="table-wrapper">
                <table class="milestone-table">
                    <thead>
                        <tr>
...
for _, row in m_poc_breakdown.iterrows():
    html_content += f"""                        <tr>
                            <td>{row['POC']}</td>
                            <td>{int(row['Critical'])}</td>
                            <td>{int(row['High'])}</td>
...
# Add total row for M POC
m_total_critical = int(m_poc_breakdown['Critical'].sum())
m_total_high = int(m_poc_breakdown['High'].sum())
m_total_medium = int(m_poc_breakdown['Medium'].sum())
m_total_low = int(m_poc_breakdown['Low'].sum())
m_total_all = int(m_poc_breakdown['Total'].sum())

html_content += f"""                        <tr style="background-color: #667eea; color: white; font-weight: bold; font-size: 1.05em;">
                            <td>Total</td>
                            <td>{m_total_critical}</td>
                            <td>{m_total_high}</td>
                            <td>{m_total_medium}</td>
                            <td>{m_total_low}</td>
                            <td>{m_total_all}</td>
                        </tr>
"""  
...
html_content += """                    </tbody>
        ...
        <div id="execution" class="tab-content">
            ...
            <div style="margin-bottom: 25px; padding: 20px; background: #f8f9fa; border-radius: 10px;">
                <div style="display: grid; grid-template-columns: repeat(2, 1fr) auto; gap: 20px; align-items: end;">
                    <div>
                        <label for="exec-sm-filter" style="font-weight: bold; font-size: 1em; margin-bottom: 8px; display: block; color: #2c3e50;">SM POC:</label>
                        <select id="exec-sm-filter" onchange="updateExecFilters('sm')" style="width: 100%; padding: 10px; font-size: 1em; border: 2px solid #667eea; border-radius: 8px; background: white; cursor: pointer;">
                            <option value="all">All SM POCs</option>
"""

# Add SM POC options
for sm_poc in unique_sm_pocs_exec:
    html_content += f"""                            <option value="{sm_poc}">{sm_poc}</option>
"""
...
for _, row in execution_summary_df.iterrows():
    html_content += f"""                    <tr class="exec-row" data-sm-poc="{row['SM POC']}" data-m-poc="{row['M POC']}" data-story-ids="{row['Story IDs']}" 
...
for _, row in testing_na_summary.iterrows():
    ...
    html_content += f"""                        <tr class="exec-row testing-na-row" data-sm-poc="{row['SM POC']}" data-m-poc="{row['M POC']}" data-testing-na-count="{count}">
                            <td>{row['SM POC']}</td>
                            <td>{row['M POC']}</td>
                            <td>{row['Node Name']}</td>
                            <td>{count}</td>
        ...
        <div id="defects" class="tab-content">
            ...
            <div class="filter-section" style="margin-bottom: 30px;">
                <div class="filter-group">
                    ...
                    <select id="defectSummarySmPocFilter" onchange="filterDefectSummary()">
                        <option value="">All</option>
"""

# Add SM POC options for defect summary filter
for sm_poc in unique_sm_pocs_defect:
    html_content += f"""                        <option value="{sm_poc}">{sm_poc}</option>
"""
...
html_content += """            
            ...
            <div class="filter-container">
                <div class="filter-group">
                    ...
                    <select id="defectSmPocFilter" onchange="updateDefectFilters()">
                        <option value="all">All SM POCs</option>
"""

# Add SM POC options for defect filter
for sm_poc in unique_sm_pocs_defect:
    html_content += f"""                        <option value="{sm_poc}">{sm_poc}</option>
"""
...
for _, row in node_breakdown_df.iterrows():
    ...
    html_content += f"""                        <tr data-sm-poc="{row['SM POC']}" data-m-poc="{row['M POC']}" 
...
for _, row in bug_summary_df.iterrows():
    ...
    html_content += f"""                        <tr data-sm-poc="{sm_poc}" data-m-poc="{m_poc}" 
        ...
        <div id="overall-defects" class="tab-content">
            ...
            <div class="filter-section">
                <div class="filter-group">
                    ...
                    <select id="overallDefectSmPocFilter" onchange="filterOverallDefects()">
                        <option value="">All</option>
"""

# Add unique SM POC options
sm_pocs = sorted(overall_defect_summary_df['SM POC'].dropna().unique())
for poc in sm_pocs:
...
html_content += """
        ...
        function updateExecFilters(changedLevel) {
            ...
            if (changedLevel === 'sm') {
                ...
                if (selectedSM !== 'all' && pocHierarchyExec[selectedSM]) {
                    const mPocs = pocHierarchyExec[selectedSM].sort();
                    mPocs.forEach(m => {
                        mFilter.innerHTML += '<option value="' + m + '">' + m + '</option>';
                    });
                    mFilter.disabled = false; // Enable M POC dropdown
                } else if (selectedSM === 'all') {
                    mFilter.disabled = true;  // Disable M POC when "All" is selected
                }
            }
        ...
        function updateDefectFilters() {
            ...
            if (selectedSM === 'all') {
                ...
                const mPocs = pocHierarchyDefect[selectedSM] || [];
                mPocs.forEach(m => {
        ...
        function filterDefectSummary() {
            ...
            // Update M POC dropdown based on selected SM POC
            if (!selectedSM) {
                // If "All" selected, disable M filter
                mFilter.disabled = true;
                mFilter.innerHTML = '<option value="">All</option>';
            } else {
                // Enable M filter and populate based on selected SM POC
                mFilter.disabled = false;
                mFilter.innerHTML = '<option value="">All</option>';
            ...
            // Determine which filter to use (M POC takes precedence, then SM)
            let filterKey = '';
            let filterValue = '';
            ...
            if (filterKey && defectSummaryData[filterKey]) {
                ...
                Object.keys(defectSummaryData).forEach(key => {
                    ...
                    if (!filterValue) {
                        ...
                        // Only count M: entries to avoid double-counting
        ...
        function filterOverallDefects() {
            ...
            // Update M POC dropdown based on selected SM POC
            if (!selectedSM) {
                // If "All" selected for SM, disable M filter
                mFilter.disabled = true;
                mFilter.innerHTML = '<option value="">All</option>';
            } else {
                // Enable M filter and populate based on selected SM POC
...
dashboard_path = DAILY_STATUS_DASHBOARD_FILE
...
# Cleanup: Remove __pycache__ folder to avoid confusion
...
if os.path.exists(pycache_path):
    try:
        ...
        print(f"Warning: Could not remove __pycache__ folder: {e}")
else:
    print("✓ No __pycache__ folder to clean")
print()
